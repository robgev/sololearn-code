...
export const getContestsInternal = () => dispatch => Service.request('Challenge/GetContestFeed').then((response) => {
	const contestsFeed = response.feed;

	const completedChallenges = contestsFeed.filter(item => item.player.status == contestTypes.Won || item.player.status == contestTypes.Lost ||
...
export const clearContests = () => dispatch => new Promise((resolve, reject) => {
	dispatch({ type: types.CLEAR_CONTESTS });
	resolve();
});

export const clearContestsInternal = () => dispatch => Service.request('Challenge/ClearContestResults')
	.then(() => dispatch(clearContests()))
	.catch((error) => {
		console.log(error);
	});

export const chooseContestCourse = courseId => dispatch => new Promise((resolve, reject) => {
	dispatch({
...
export const getAllPlayersInternal = (name, courseId) => dispatch => Service.request('Challenge/FindPlayers', { name, courseId })
	.then((response) => {
...
export const emptyAllPlayers = () => dispatch => new Promise((resolve, reject) => {
	dispatch({
...
export const getContestInternal = id => dispatch => Service.request('Challenge/GetContest', { id })
	.then((response) => {
