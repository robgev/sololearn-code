...
const NoteBlock = (props) => {
    return (
        <div className="note-block" style={styles.noteBlock}>
            <span className="note" style={styles.note} dangerouslySetInnerHTML={{ __html: props.noteText }}></span>
        </div>
    );
}
...
class QuizText extends Component {
    ...
    openTooltip(e, glossaryItem, placement) {
        let tooltipContent = glossaryItem.parentNode;
        let tooltip = tooltipContent.querySelector('.tooltip');

        let result = getPosition(e, e.target, tooltip, placement, "solid", {}, true);
    ...
    getTootlip(text, tooltipText) {       
        ...
        return (
            <span className="tooltip-content" style={styles.tooltipWrapper} data-text={tooltipText} dangerouslySetInnerHTML={{ __html: innerHtml }}></span>
        );
    }
    ...
    generateTooltips(text) {
        ...
        if (glossary != null) {
            for (let i = 0; i < glossary.length; i++) {
                let currentTerms = glossary[i].terms;
                    ...
                    if (currentTerms[j].pattern != null) {
                        ...
                        if (!(pattern.indexOf('(?<') > -1)) {
                            ...
                            text = text.replace(patternRegex, (...params) => {
                                ...
                                let tooltip = this.getTootlip(matchedString, currentTerms[j].text);
                                const tooltipHTML = ReactDOMServer.renderToStaticMarkup(tooltip);
