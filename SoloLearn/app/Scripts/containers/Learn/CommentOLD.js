...
class Comment extends Component {
        ...
        return this.comment.replies.map((reply, index) => {
            return (
                <Comment 
                    ...
                    editOpened={this.props.editState && reply.id == this.props.activeComment.id}
    ...
    getPrimaryControls() {
        ...
        return(
           <div className="primary-controls" style={styles.commentControls.right}>
                ...
                { true && <FlatButton label="Edit" primary={true} onClick={() => this.props.openEdit(this.comment.id, this.comment.parentID) } /> }
    ...
    getEditControls() {
        ...
        return(
            <div className="edit-controls" style={styles.commentControls.right}>
                ...
                <FlatButton label="Cancel" onClick={() => this.cancelEditing() } />
    ...
    getEditableArea() {
        return (
            <div style={styles.commentContent}>
                {!this.props.editOpened && <div className="original-message" dangerouslySetInnerHTML={{ __html: this.updateMessage(this.state.textFieldValue) }} style={styles.commentMessage}></div> }
                { (true && this.props.editOpened) && 
    ...
    render() {
        if(!this.isReply) {
            return (           
                <div className="comment-container" style={this.props.activeComment.parentId == this.comment.id ? [styles.commentContainer.base, styles.commentContainer.elevated] : styles.commentContainer.base}>
                    <div className="primary comment" ref={"comment" + this.comment.id} style={this.props.activeComment.id == this.comment.id ? [styles.comment.base, styles.comment.elevated] : styles.comment.base}>
                        <div className="content" style={styles.commentConent}>
                            <Avatar size={50} style={styles.avatar}>{this.comment.userName.charAt(0)}</Avatar>
                            <div className="comment-details" style={this.props.editOpened ? [styles.commentDetails.base, styles.commentDetails.editing] : styles.commentDetails.base}>
                        ...
                        <div className="controls" style={styles.commentControls.base}>
                            ...
                            { (true && this.props.editOpened) && this.getEditControls() }
                            { (!this.props.editOpened) && this.getPrimaryControls() }
                        </div>
                    </div>
                    {
            ...
            return ( 
                <div className="secondary comment" ref={"comment" + this.comment.id} style={this.props.activeComment.id == this.comment.id ? getStyles(styles.comment.base, styles.comment.elevated) : styles.comment.base}>
                    <div className="content" style={styles.commentConent}>
                        <Avatar size={50} style={styles.avatar}>{this.comment.userName.charAt(0)}</Avatar>
                        <div className="comment-details" style={this.props.editOpened ? getStyles(styles.commentDetails.base, styles.commentDetails.editing) : styles.commentDetails.base}>
                            <span className="name" style={styles.userName}>{this.comment.userName}</span>
                            <p className="date" style={styles.commentDate}>{this.comment.date}</p>
                            {this.getEditableArea()}
                        </div>
