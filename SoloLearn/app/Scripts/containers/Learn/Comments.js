...
class Comments extends Component {
    constructor(props) {
        ...
        this.closeReplyBoxToolbar = this.closeReplyBoxToolbar.bind(this);
    ...
    loadComments() {
        ...
        this.loadCommentsInternal(this.props.quizId, this.props.type, null, this.state.comments.length, 20, this.state.ordering).then(response => {
            if(response.comments.length > 0) {
                let loadedComments = this.state.comments.slice();
    ...
    renderComments() {
        console.log("comments --> render", this.state.comments.length);
    ...
    handleScroll(e, scrollableArea) {
        if(!this.state.fullyLoaded &&  !this.state.isLoading && this.state.comments.length > 0 && scrollableArea.scrollTop === (scrollableArea.scrollHeight - scrollableArea.offsetHeight))
        {
            this.loadComments();
        }
    }
    ...
    openReplyBoxToolbar(id, parentId, userName, isReply) {
        console.log("openReplyBoxToolbar");
    ...
    closeReplyBoxToolbar() {
        console.log("closeReplyBoxToolbar");
        this.setState({
            isReplying: false,
            activeComment: {
                id: null,
    ...
    reply(message) {
        ...
        let parentId = this.state.isReplying ? this.state.activeComment.parentId : this.state.activeComment.id;
        ...
        return (
            <div>
                <Dialog id="comments-container" 
                    ...
                    actions={<ReplyBox isPrimary={!this.state.isReplying} defaultText={this.state.isReplying ? "@" + this.state.activeComment.userName + " " : ""} userName={this.state.activeComment.userName} closeToolbar={this.closeReplyBoxToolbar} reply={this.reply}/>}
    ...
    //    return this.state.comments.length == nextState.comments.length;
    //}
}
