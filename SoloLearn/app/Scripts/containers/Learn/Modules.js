...
class Modules extends Component {
            ...
            return (
                ...
                <div className={"module " + alignmentClass} style={module.alignment == 1 ? styles.module.base : [styles.module.base, styles.module[alignmentClass]]} key={module.id}>
                    <Link style={getSyles(styles.moduleContent.base, styles.moduleContent[alignmentClass])} className="content" to={"/learn/" + this.props.params.courseName + "/" + module.id + "/" + toSeoFrendly(module.name, 100)} onClick={(e) => this.handleClick(e, module.id, moduleState)}>
                        <Paper className={"module-circle " + stateClass}  style={getSyles(styles.moduleCircle.base, styles.moduleCircle[stateClass])} zDepth={1} circle={true} key={module.id}>
    ...
    renderCourses() {
        ...
        let userCoursesIds = userCourses.map(item => item.id);

        let difference = userCoursesIds.map((id, index) => {
                            ...
                            userCourses.map((course, index) => {
                                return (
                                    [<div key={course.id} style={styles.course}>
                                        ...
                                        <IconMenu style={styles.courseActions}
    ...
    selectCourse(courseId, addToCourses) {
        let alias = this.props.courses.find(item => item.id == courseId).alias
    ...
    resetProgress(courseId) {
        ...
        let skills = this.props.userProfile.skills;
        let index = skills.findIndex(item => item.id == courseId);
        skills[index].progress = 0;

        this.props.toggleCourse(skills);
        Progress.reset();
        Service.request("ResetProgress", { courseId: courseId });
    }
        ...
        return (
            ...
            {   userCourses.length > 0 ?
                ...
                <div className="no-courses" style={styles.noCourses}>
                    ...
                    <RaisedButton style={styles.addCoursesButton} label="Add Courses" secondary={true} onClick={() => this.openCourseSelect()}/>
                </div>
            }
            { 
                ...
                <Dialog id="courses" 
                    ...
                    onRequestClose={this.closeCourseSelect}>{this.renderCourses()}</Dialog>
            }
