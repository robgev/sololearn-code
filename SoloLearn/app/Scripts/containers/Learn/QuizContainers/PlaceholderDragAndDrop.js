...
class PlaceholderDragAndDrop extends Component {
    ...
    setAnswerPositions() {
        ...
        for (let i = 0; i < answers.length; i++) {
            ...
            let answerDOMElement = ReactDOM.findDOMNode(this.refs["answer" + answer.id]);
            let answerPostions = this.offset(answerDOMElement);
    ...
    move(placeholder, answer) {
        let placeholderPositions = this.offset(placeholder);
    ...
    unlock() {
        ...
        let correctAnswers = this.props.quiz.answers.filter((quiz) => { return quiz.isCorrect; });
        ...
        for (let i = 0; i < correctAnswers.length; i++) {
            ...
            let placeholder = ReactDOM.findDOMNode(child["_child" + index].refs["placeholder" + index]);
    ...
    componentDidMount() {
        ...
        let bag = ReactDOM.findDOMNode(this.refs.bottom);

        let options = { };
        let drake = Dragula(refs, options);
