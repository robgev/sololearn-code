...
class Playground extends Component {
	...
	loadEditor() {
		const sample = (!this.isUserCode && !this.isCodeTemplate) ? texts[this.state.mode] : this.sourceCode;
		const editorMode = `ace/mode/${this.state.mode}`;
		const theme = `ace/theme/${this.state.theme}`;
	...
	handleLanguageChange(e, index, value) {
		const type = editorSettings[value].type;
		const language = editorSettings[value].language;
	...
	resetEditorValue() {
		...
		const language = editorSettings[this.state.mode].language;
		let sample = '';
		const userCodeOpened = this.isUserCode && language == this.userCodeData.language;
	...
	insertToHead(source, value) {
		...
		const sourceCode = `${htmlCode.substring(0, headEnd)}\t\t${value}${htmlCode.substring(headEnd)}`;
	...
	checkForInput(lanugage) {
		...
		if (lanugage == 'py') {
			const codeBlock = this.sourceCode.replace(/(([^'"])(#)|(^#))((.*)$)/gm, ''); // $2
			const inputRegex = inputRegexes[lanugage];
	...
	runCode() {
		...
		const lanugage = editorSettings[this.state.mode].language;
		...
		} else if (this.checkForInput(lanugage)) {
	...
	save(createNewCode) {
		...
		} else if (this.isUserCode && this.userCodeData.userID == 24379) { // MY USERID
			this.setState({
				isSaving: true,
				snackBarOpened: true,
			});
	...
	submitSave() {
		...
		this.saveCodeInternal(0).then((response) => {
			...
			const url = `http://www.sololearn.com:16680/playground/${code.publicID}/${code.language}`; // TODO CHANGE TO SOLOLEARN's URL
			window.location = url;
		}).catch((error) => {
			console.log(error);
		});
    ...
    handleSourceFilterChange(e, index, value) {
    	const sourceUrl = value == 'none' ? '' : externalResources[value];
	...
	/>,
		...
		{this.state.gettingCode && <LoadingOverlay />}
    			...
    			<Paper id="playground" style={!this.state.gettingCode ? styles.playground.base : styles.playground.hide}>
    				{aceEditorStyle}
		{aceLineStyle}
    				{this.isUserCode && <div className="user-details">{this.getUserDetails()}</div>}
		<div className="tabs" style={styles.tabs}>
    					{this.generateTabs()}
	</div>
    				<div id="editor" ref="editor" style={!showWebOutput ? styles.editor.base : [ styles.editor.base, styles.editor.hide ]} />
	...
	<iframe id="output-frame" frameBorder="0" style={styles.webIframe} />
    ...
    >
    	...
    	<TextField id="inputs" style={styles.inputStyle} multiLine fullWidth value={this.state.inputs} maxLength={100} onChange={this.handleInputsChange} />
    ...
    >
	<p style={styles.popupSubTitle}>{texts.savePopupSubTitle}</p>
    	<TextField id="codeName" style={styles.inputStyle} fullWidth value={this.state.codeName} hintText="Code Name:" maxLength={100} errorText={this.state.errorText} onChange={this.handleCodeNameChange} />
    ...
    >
    	...
    	<TextField id="inputs" style={styles.inputStyle} fullWidth hintText="External resource url" disabled={this.state.selectedSource != 'none'} value={this.state.sourceUrl} maxLength={100} onChange={this.handleSourceUrlChange} />
    </Dialog>
    			}
    			<Snackbar
