            ...
            >
                {
                    nodes.map((node, index) =>{
                        return (texts[node.key] ? texts[node.key] : "").replace("{placeholder}", node.replacemant);
                    })
                }
            </Dialog>
        );
        ...
        return (
            <div className="check-popup-overlay" style={styles.checkPopupOverlay}>
                ...
                { isCorrect ? 
                    ...
                    isCheckpoint && <RaisedButton key="backToTextButton" label="Back" primary={true} style={styles.backButton} labelStyle={styles.backButtonLabel} onClick={actionCallback} />]
                }
                </Paper>
            </div>
        );
