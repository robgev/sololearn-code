...
class ProgressManager {
    ...
    loadCourse(course) {
        ...
        for (let i = 0; i < this.modules.length; i++) {
            let state = this.moduleStates['m' + this.modules[i].id];
            if (state != null) {
    ...
    propagateModuleProgress(module, state) {
        ...
        state.totalLessons = Object.keys(module.lessons).filter(function(lessonId) { return module.lessons[lessonId].type == 0 }).length;
        const passedItems = Object.keys(module.lessons).filter(function(lessonId) {       
            return that.getLessonStateById(module.lessons[lessonId].id).visualState == ProgressState.Normal 
        });
        state.passedItems = passedItems.length;
        state.passedLessons = passedItems.filter(function (l) { return l.type == 0 }).length;
        ...
        if(state.passedItems == 0)
        {
            ...
            else if (moduleIndex > 0)
            {
                ...
                if (Object.keys(prevModule.lessons).filter(function (lessonId) { return that.getLessonStateById(prevModule.lessons[lessonId].id).visualState == ProgressState.Normal  }).length == prevModule.lessons.length)
                {
                    state.visualState = ProgressState.Active;
                }
                else
    ...
    highlightActiveLesson() {
        ...
        for (let i = 0; i < this.lessons.length; i++) {
            let lessonState = this.getLessonState(this.lessons[i]);
    ...
    propagateLessonProgress(lessonProgress, state) {
        ...
        if(module != null){
            let moduleState = this.moduleStates['m' + module.id] || null;
            if (moduleState != null) {
    ...
    getLessonState(lesson) {
        ...
        if (!state) {
            ...
            let progress = this.getLessonProgress(lesson.id);
            if (progress) {
    ...
    addResult(lessonID, quizID, isSuccessful, time) {
        ...
        {
            let wasCompleted = lessonProgress.isCompleted;
    ...
    {
        let currentPoints = this.progress.points;
